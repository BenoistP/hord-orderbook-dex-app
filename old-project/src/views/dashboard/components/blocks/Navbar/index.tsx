...
const Navbar = ({ blockValues, lastTradePrice, lastTradePriceType, account }: Props) => {
  ...
  <S.Wrapper>
    <S.WrapperInfo>
      ...
      <S.ContainerInfo>
          <NavbarItem label="Last Trade Price (BUSD)" info={Dinero({ amount: Math.round(lastTradePrice * 100) }).toFormat('$0,0.00')} type={lastTradePriceType} />
        <NavbarItem label="Block Price %" info={blockValues.blockPrice.toString()} />
        <NavbarItem label="Block Volume (BUSD)" info={Dinero({ amount: Math.round(blockValues.volume * 100) }).toFormat('$0').toString().slice(0, 6)} />
        <S.WrapperVolume>
          <S.VolumeHigh>
            <S.Span>
              Block High
