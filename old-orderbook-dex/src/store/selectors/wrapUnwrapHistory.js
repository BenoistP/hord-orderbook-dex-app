...
const getTokenWrapUnwrapHistoryStatus = createSelector(wrapUnwrapHistory, (s) => s.get('historyLoadingStatus'));
...
const hasAccountEntry = createSelector(wrapUnwrapHistory, reselect.getProps, (s, account) =>
  s.hasIn(['wrapUnwrapHistory', account]),
);

const tokenWrapUnwrapHistory = createSelector(wrapUnwrapHistory, accounts.defaultAccount, (s, defaultAccount) =>
  s.hasIn(['wrapUnwrapHistory', defaultAccount])
    ? s.getIn(['wrapUnwrapHistory', defaultAccount]).sort((p, n) => (p.get('timestamp') > n.get('timestamp') ? -1 : 1))
    : fromJS([]),
);
