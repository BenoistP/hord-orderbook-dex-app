...
const activeTradingPairOffersInitialLoadStatus = createSelector(
  ...
  (state, activeTradingPair) =>
    state.hasIn(['offers', Map(activeTradingPair), 'initialSyncStatus'])
...
const activeTradingPairBestBuyOfferId = createSelector(offers, (state) =>
  state.getIn(['activeTradingPairBestOfferId', 'bestBuyOfferId']),
);

const activeTradingPairBestSellOfferId = createSelector(offers, (state) =>
  state.getIn(['activeTradingPairBestOfferId', 'bestSellOfferId']),
);
...
const reSyncOffersSet = createSelector(offers, (s) => s.get('reSyncOffersSet'));
