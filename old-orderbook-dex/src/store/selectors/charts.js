...
const priceChartTrades = createSelector(tokenTrades, (trades) => {
  ...
  const res = (trades || []).filter((t) => t.timestamp >= since);
  return res.length == 0 ? [] : res.toJS();
});

const priceChartLabels = createSelector(priceChartTrades, (priceChartTrades) =>
...
const offersBids = createSelector(offers, reselect.getProps, (offers, props) =>
  !props
    ? []
    : removeOutliersFromArray(
        (
          (
            offers.get(
              Map({
                baseToken: props.tradingPair.baseToken,
                quoteToken: props.tradingPair.quoteToken,
              }),
            ) || Map()
          ).get('buyOffers') || List()
        )
          .sortBy((b) => b.bid_price_sort)
          .toJS(),
        'bid_price_sort',
        3,
      ),
...
const offersAsks = createSelector(offers, reselect.getProps, (offers, props) =>
  !props
    ? []
    : removeOutliersFromArray(
        (
...
function upSum(array) {
  return array.reduce(
    ({ sum, result }, bn) => ({
      sum: sum.add(bn),
      result: result.concat(bn.add(sum)),
    }),
    { sum: new BigNumber(0), result: [] },
  ).result;
}
...
function downSum(array) {
  return array.reduceRight(
    ({ sum, result }, bn) => ({
      sum: sum.add(bn),
      result: [bn.add(sum)].concat(result),
    }),
    { sum: new BigNumber(0), result: [] },
  ).result;
}
...
const depthChartData = createSelector(offersBids, offersAsks, (bids, asks) => {
  ...
  const askAmountsData = mapWithPreviousResult(vals, (val, prevResult) => {
    ...
    if (index !== -1) {
      ...
      return {
        ...
        tooltip: {
          quote: askAmounts.quote[index],
          base: askAmounts.base[index],
        },
      };
    } else if (
      askPrices.length === 0 ||
...
const depthChartTooltips = createSelector(depthChartData, (depthChartData) => ({
