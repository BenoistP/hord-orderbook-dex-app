...
export const fetchEthereumPrice = createAction('NETWORK/FETCH_ETHEREUM_PRICE', () =>
 ...
 */
export const getLatestBlockNumber = createAction('NETWORK/GET_LATEST_BLOCK_NUMBER', async () =>
 ...
 * @dev We get latest mined block number
 */
export const setLatestBlockNumber = createAction('NETWORK/SET_LATEST_BLOCK_NUMBER', (latestBlockNumber) => ({
  latestBlockNumber,
  latestBlockReceivedAt: getTimestamp(),
}));

export const getBlock = createAction('NETWORK_GET_BLOCK', async (blockNumberLatestOrPending) =>
  web3p.eth.getBlock(blockNumberLatestOrPending),
);

export const getLatestBlock = createAction('NETWORK_GET_LATEST_BLOCK', async () => web3p.eth.getBlock('latest'));

/**
 * @dev Here we create 3 actions for checking the network status
 * @type {{pending, fulfilled, rejected}|*}
 */
const subscribeLatestBlockFilter = createPromiseActions('NETWORK/SUBSCRIBE_LATEST_BLOCK_FILTER');
export const subscribeLatestBlockFilterEpic = () => async (dispatch, getState) => {
  ...
  const update = () => {
    ...
    if (areContractsInitialized && tokens.activeTradingPair(getState())) {
      if (offers.activeTradingPairOffersInitiallyLoaded(getState())) {
        ...
        dispatch(
          balancesReducer.actions.syncTokenBalances(getTokenContractsList(), accounts.defaultAccount(getState())),
        );
        dispatch(
