...
export const checkNetworkInitialEpic = () => async (dispatch, getState) => {
  ...
  if (!tokens.activeTradingPair(getState())) {
    dispatch(tokensReducer.actions.setActiveTradingPairEpic(tokens.defaultTradingPair(getState()).toJS(), false));
  }
  ...
  try {
    ...
    ]).then(onNetworkCheckEndEpic(dispatch, getState, !platform.allInitialSubscriptionsRegistered(getState())));
  } catch (e) {
