...
const syncTransaction = createAction('TX/SYNC_TRANSACTION', (txHash) => web3p.eth.getTransactionReceipt(txHash));

const transactionRejected = createAction('TX/TRANSACTION_REJECTED', (err) => err);

const getCurrentGasPrice = createAction('TX/GET_CURRENT_GAS_PRICE', async () => web3p.eth.getGasPrice());

const getCurrentTxNonce = createAction('TX/GET_CURRENT_TX_NONCE', async (accountAddress) =>
...
const reducer = handleActions(
  {
    [addTransaction.pending]: (state, { payload: { txHash, txType, txStats, txMeta } }) => {
    ...
    [addTransaction.fulfilled]: (state, { payload: { txHash, txReceipt, txStatus, txStats } }) => {
    ...
    [addTransaction.rejected]: (state, { payload: { txHash, txReceipt, txStatus, txStats } }) => {
      const txListIndex = state.get('txList').findIndex((tx) => tx.get('txHash') === txHash);
      return state.updateIn(['txList', txListIndex], (transaction) =>
        transaction
          .set('txReceipt', txReceipt)
