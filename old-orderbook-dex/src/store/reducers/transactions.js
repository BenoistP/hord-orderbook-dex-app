...
export const TX_OFFER_PARTIALLY_FULFILLED =
  TX__GROUP__OFFERS + 'OFFER_FULFILLED_PARTIALLY';
export const TX_OFFER_FULFILLED_COMPLETELY =
  TX__GROUP__OFFERS + 'OFFER_FULFILLED_COMPLETELY';
...
export const TX_UNWRAP_TOKEN_WRAPPER =
  TX__GROUP__WRAP_UNWRAP + 'UNWRAP_TOKEN_WRAPPER';

export const TX_STATUS_AWAITING_USER_ACCEPTANCE =
  'TX/STATUS_AWAITING_USER_ACCEPTANCE'; // PENDING
export const TX_STATUS_AWAITING_CONFIRMATION =
  'TX/STATUS_AWAITING_CONFIRMATION'; // PENDING
...
export const TX_ALLOWANCE_TRUST_TOGGLE =
  TX__GROUP__LIMITS + 'TX/ALLOWANCE_TRUST_TOGGLE';
...
const init = createAction('TX/INIT', () => null);
...
const syncTransaction = createAction('TX/SYNC_TRANSACTION', (txHash) =>
...
const getCurrentGasPrice = createAction('TX/GET_CURRENT_GAS_PRICE', async () =>
...
const getCurrentTxNonce = createAction(
  ...
  async (accountAddress) =>
    web3p.eth.getTransactionCount(accountAddress || web3.eth.defaultAccount),
);
...
const reducer = handleActions(
  {
    [addTransaction.pending]: (
      state,
      { payload: { txHash, txType, txStats, txMeta } },
    ) => {
      ...
      return state.updateIn(['txList'], (txList) => txList.push(txPayload));
    },
    [addTransaction.fulfilled]: (
      state,
      { payload: { txHash, txReceipt, txStatus, txStats } },
    ) => {
      const txListIndex = state
        .get('txList')
        .findIndex((tx) => tx.get('txHash') === txHash);
      return state.updateIn(['txList', txListIndex], (transaction) =>
        transaction
          ...
          .setIn(
            ['txStats', 'txTotalTimeSec'],
            txStats.txEndTimestamp - transaction.getIn('txStartTimestamp'),
          ),
      );
    },
    ...
    [fulfilled(getCurrentTxNonce)]: (state, { payload }) =>
      state.set('txNonce', parseInt(payload) + 1),
    [fulfilled(getCurrentGasPrice)]: (state, { payload }) =>
      state.set('currentGasPriceInWei', payload.toString()),
  },
  initialState,
);
