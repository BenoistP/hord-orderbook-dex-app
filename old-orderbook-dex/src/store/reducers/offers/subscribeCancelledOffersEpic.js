  ...
  async (dispatch, getState) => {
    ...
    try {
      doGetMarketContractInstance()
        .LogKill(filter, { fromBlock, toBlock: 'latest' })
        .then((err, LogKillEvent) => {
          const { id, maker, timestamp } = LogKillEvent.args;
      ...
      switch (offerType) {
        case TYPE_BUY_OFFER:
          return tradingPairOffers.updateIn(['buyOffers'], (buyOffers) =>
            buyOffers.update(
              buyOffers.findIndex((buyOffer) => buyOffer.id === offerId),
              (offerToUpdate) => {
        ...
        case TYPE_SELL_OFFER:
          return tradingPairOffers.updateIn(['sellOffers'], (sellOffers) =>
            sellOffers.update(
              sellOffers.findIndex((sellOffer) => sellOffer.id === offerId),
              (offerToUpdate) => {
                return { ...offerToUpdate, status: OFFER_STATUS_INACTIVE };
              },
            ),
