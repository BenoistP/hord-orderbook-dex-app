...
import { MAKE_BUY_OFFER, MAKE_SELL_OFFER, DEFAULT_GAS_PRICE } from './../../constants';
...
const buyMaxEpic =
  (offerMakeType, { activeQuoteTokenBalance = balances.activeQuoteTokenBalance } = {}) =>
  (dispatch, getState) => {
...
const sellMaxEpic =
  (
    offerMakeType,
    { activeBaseTokenBalance = balances.activeBaseTokenBalance, currentFormValues = offerMakes.currentFormValues } = {},
  ) =>
  (dispatch, getState) => {
    ...
    const totalSerialized = usersBaseTokenBalanceBN.mul(priceBN).toFixed().toString();
...
const makeOfferTransaction = createAction(
  ...
  ({ payAmount, payToken, buyAmount, buyToken, isCloseMatchingEnabled, rankIndex, gasPrice = DEFAULT_GAS_PRICE }) => {
...
const setOfferMakeModalOpen = createAction('OFFER_MAKES/SET_MODAL_OPEN', (offerMakeType) => ({
  formName: offerMakeToFormName(offerMakeType),
}));
...
const setOfferMakeModalClosed = createAction('OFFER_MAKES/SET_MODAL_CLOSED', (offerMakeType) => ({
  formName: offerMakeToFormName(offerMakeType),
}));
...
const priceFieldChangedEpic =
  (offerMakeType, value, { formValueSelector = localFormValueSelector } = {}) =>
  (dispatch, getState) => {
    ...
    if (!value || isNaN(value) || parseFloat(value) === 0 || isNaN(volume) || parseFloat(volume) === 0) {
      ...
      dispatch(localChange(offerMakeType, 'total', web3.toBigNumber(volume).mul(value).toFixed().toString()));
    }
...
const volumeFieldValueChangedEpic =
  ...
  (dispatch, getState) => {
    ...
    if (isNaN(value) || parseFloat(value) <= 0 || isNaN(price) || parseFloat(price) === 0) {
      ...
      dispatch(localChange(offerMakeType, 'total', web3.toBigNumber(value).mul(price).toFixed().toString()));
    }
    dispatch(defer(updateTransactionGasCostEstimateEpicThrottled, offerMakeType));
  };
...
const totalFieldValueChangedEpic =
  ...
  (dispatch, getState) => {
    const price = formValueSelector(offerMakeType)(getState(), 'price');
    if (!isNaN(value) && parseFloat(value) > 0 && !isNaN(price) && parseFloat(price) > 0) {
      dispatch(localChange(offerMakeType, 'volume', web3.toBigNumber(value).div(price).toFixed().toString()));
      dispatch(defer(updateTransactionGasCostEstimateEpicThrottled, offerMakeType));
    } else {
    ...
    new Promise((resolve, reject) => {
      getMarketNoProxyContractInstance().offer['uint256,address,uint256,address,uint256,bool'].estimateGas(
        ...
        (e, estimation) => {
          if (e) {
            reject({
              payAmount: convertToTokenPrecision(payAmount, getTokenByAddress(payToken)),
              payToken,
              buyAmount: convertToTokenPrecision(buyAmount, getTokenByAddress(buyToken)),
              buyToken,
              toAddress,
            });
          } else {
...
const setGasExceedsTheLimitEnabled = createAction('OFFER_MAKES/GAS_EXCEEDS_THE_LIMIT_ENABLED');
const setGasExceedsTheLimitDisabled = createAction('OFFER_MAKES/GAS_EXCEEDS_THE_LIMIT_DISABLED');

const updateTransactionGasCostEstimateEpic =
  (
    offerMakeType,
    { canMakeOffer = offerMakes.canMakeOffer, activeOfferMakePure = offerMakes.activeOfferMakePure } = {},
  ) =>
  async (dispatch, getState) => {
    ...
    if (isNumericAndGreaterThanZero(payAmount) && isNumericAndGreaterThanZero(buyAmount)) {
      const transactionGasCostEstimate = (
        await dispatch(defer(getTransactionGasEstimate, payAmount, payToken, buyAmount, buyToken, toAddress))
      ).value;
...
const reducer = handleActions(
  {
    ...
    [pending(getTransactionGasEstimate)]: (state) =>
      state.set('transactionGasCostEstimateError', false).set('transactionGasCostEstimatePending', true),
    ...
    [rejected(getTransactionGasEstimate)]: (state) =>
      state.set('transactionGasCostEstimateError', true).set('transactionGasCostEstimatePending', false),
    ...
    [setGasExceedsTheLimitEnabled]: (state) => state.set('exceededGasLimit', true),
    [setGasExceedsTheLimitDisabled]: (state) => state.set('exceededGasLimit', false),
  },
  initialState,
);
