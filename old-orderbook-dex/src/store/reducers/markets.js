...
const eventLogIsMatchingEnabled = createAction(EVENT___LOG_IS_MATCHING_ENABLED, (isEnabled) => isEnabled);

const subscribeLogMatchingEnabled = createPromiseActions(SUBSCRIBE___LOG_IS_MATCHING_ENABLED_EVENT);
const subscribeLogMatchingEnabledEpic = () => async (dispatch) => {
  ...
  getMarketContractInstance()
    ...
    .then((status) => dispatch(eventLogIsMatchingEnabled(status.args.isEnabled)))
...
const setActiveMarketAddress = createAction(SET_ACTIVE_MARKET_ADDRESS, (address) => address);

const checkIfBuyEnabled = createAction(CHECK_IF_BUY_ENABLED, () => getMarketContractInstance().buyEnabled());
