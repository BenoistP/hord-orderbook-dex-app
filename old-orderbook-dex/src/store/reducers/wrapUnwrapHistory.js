...
const loadingWrapUnwrapHistorySetPending = createAction('WRAP_UNWRAP_HISTORY/LOADING_WRAP_UNWRAP_HISTORY_SET_PENDING');
...
const loadingWrapUnwrapHistorySetPaused = createAction('WRAP_UNWRAP_HISTORY/LOADING_WRAP_UNWRAP_HISTORY_SET_PAUSED');

const loadingWrapUnwrapHistorySetCompleted = createAction(
  'WRAP_UNWRAP_HISTORY/LOADING_WRAP_UNWRAP_HISTORY_SET_COMPLETED',
);

const loadWrapUnwrapsHistory$ = createPromiseActions('WRAP_UNWRAP_HISTORY/LOAD_WRAP_UNWRAP_HISTORY');

const initializeAccountList = createAction('WRAP_UNWRAP_HISTORY/INITIALIZE_ACCOUNT_LIST');
...
const loadEtherWrapUnwrapsHistoryEpic = (address, config) => async (dispatch, getState) => {
  ...
  const activeMarketConfig = marketConfig[network.activeNetworkName(getState())];
  ...
  registerAccountSpecificSubscriptions({
    wrapUnwrapHistoryEventSub: {
      ...
      value: tokenContract.Deposit({ who: filterAddress }, filterConfig).then(async (err, ethWrapEvent) => {
        const blockInfo = (await dispatch(networkReducer.actions.getBlock(ethWrapEvent.blockNumber))).value;
  ...
  registerAccountSpecificSubscriptions({
    wrapUnwrapHistoryEventSub: {
      ...
      value: tokenContract.Withdrawal({ who: filterAddress }, filterConfig).then(async (err, ethUnwrapEvent) => {
        const blockInfo = (await dispatch(networkReducer.actions.getBlock(ethUnwrapEvent.blockNumber))).value;
...
const loadGNTWrapUnwrapsHistoryEpic = (address, config) => async (dispatch, getState) => {
  ...
  registerAccountSpecificSubscriptions({
    wrapUnwrapHistoryEventSub: {
      ...
      value: tokenContract
        .Transfer({ from: GNTBrokerAddress, to: WGNTContract.address }, { fromBlock })
        .then(async (err, wrapUnwrapEvent) => {
          const blockInfo = (await dispatch(networkReducer.actions.getBlock(wrapUnwrapEvent.blockNumber))).value;
          ...
          dispatch(
            tokenWrapEvent(tokenName, wrapUnwrapEvent.transactionHash, filterAddress, wrapUnwrapEvent, blockInfo),
          );
        }),
    },
  });
  ...
  registerAccountSpecificSubscriptions({
    wrapUnwrapHistoryEventSub: {
      ...
      value: tokenContract
        .Transfer({ from: WGNTContract.address, to: filterAddress }, filterConfig)
        .then(async (err, wrapUnwrapEvent) => {
          const blockInfo = (await dispatch(networkReducer.actions.getBlock(wrapUnwrapEvent.blockNumber))).value;
          ...
          dispatch(
            tokenUnwrapEvent(tokenName, wrapUnwrapEvent.transactionHash, filterAddress, wrapUnwrapEvent, blockInfo),
          );
        }),
    },
  });
...
const reducer = handleActions(
  {
    ...
    [loadingWrapUnwrapHistorySetInitiallyLoaded]: (state) =>
      state.set('historyLoadingStatus', WRAP_UNWRAP_HISTORY_LOAD_STATUS_LOAD_STATUS_INITIALLY_LOADED),
