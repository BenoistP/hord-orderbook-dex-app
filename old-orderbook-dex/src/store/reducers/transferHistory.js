...
export const TRANSFER_HISTORY_LOAD_STATUS_INITIALLY_LOADED = 'TRANSFER_HISTORY/LOAD_STATUS_INITIALLY_LOADED';
...
const tokenTransferEvent = createAction(
  ...
  (tokenName, transactionHash, userAddress, event, blockInfo, transferType, accountAddress) => ({
    tokenName,
    userAddress,
    event,
    blockInfo,
...
const initializeAccountList = createAction('TRANSFER_HISTORY/INITIALIZE_ACCOUNT_LIST');

const loadTokenTransfersHistory = createPromiseActions('TRANSFER_HISTORY/LOAD_TOKEN_TRANSFER_HISTORY');

const loadingTokenTransferHistorySetPending = createAction(
  'TRANSFER_HISTORY/LOADING_TOKEN_TRANSFER_HISTORY_SET_PENDING',
);

const loadingTokenTransferHistorySetPaused = createAction('TRANSFER_HISTORY/LOADING_TOKEN_TRANSFER_HISTORY_SET_PAUSED');
...
const loadTokenTransfersHistoryEpic = (tokenName, address, config) => async (dispatch, getState) => {
  ...
  } else if (transferHistory.getTokenTransferHistoryStatus(getState(), tokenName)) {
    return;
  }
  ...
  const handleTransferEvent = (transferType) => async (err, transferEvent) => {
    const blockInfo = (await dispatch(networkReducer.actions.getBlock(transferEvent.blockNumber))).value;
    dispatch(
      tokenTransferEvent(
        tokenName,
...
const reducer = handleActions(
  {
    ...
    [tokenTransferEvent]: (state, { payload: { tokenName, event, blockInfo, transferType, accountAddress } }) => {
