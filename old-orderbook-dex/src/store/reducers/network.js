...
import { checkNetworkEpic, getConnectedNetworkId } from './network/checkNetworkEpic';
import { CheckNetworkAction } from './network/CheckNetworkAction';
import { setLastNetworkCheckStartAt, setLastNetworkCheckEndAt } from './network/onNetworkCheckEndEpic';
...
const reducer = handleActions(
  {
    ...
    [setWaitingForNetworkAccess]: (state, { payload }) => state.set('waitingForNetworkAccess', payload),
    [setLastNetworkCheckStartAt]: (state, { payload }) => state.setIn(['lastNetworkCheckAt', 'start'], payload),
    [setLastNetworkCheckEndAt]: (state, { payload }) => state.setIn(['lastNetworkCheckAt', 'end'], payload),
    [CheckNetworkAction.pending]: (state) => state.set('isNetworkCheckPending', true),
    [CheckNetworkAction.fulfilled]: (state) => state.set('isNetworkCheckPending', false),
    [CheckNetworkAction.rejected]: (state) => state.set('isNetworkCheckPending', false),
    [connected]: (state) => state.set('status', ONLINE).set('isConnecting', false).set('connected', true),
    [connecting]: (state) => state.set('isConnecting', true).set('status', CONNECTING),
    [disconnected]: (state) => state.set('status', CLOSED).set('isConnecting', false).set('connected', false),
    [syncNetwork.pending]: (state) => state.setIn(['sync', 'isPending'], true).set('status', OUT_OF_SYNC),
    [syncNetwork.fulfilled]: (state) => state.setIn(['sync', 'isPending'], false).set('status', ONLINE),
    [fulfilled(getConnectedNetworkId)]: (state, { payload }) =>
      state.update('activeNetworkId', (nid) => (!!payload && nid === payload ? nid : payload)),
    [fulfilled(getLatestBlockNumber)]: (state, { payload: { latestBlockNumber, latestBlockReceivedAt } }) =>
      state.update('latestBlockNumber', () => latestBlockNumber).set('latestBlockReceivedAt', latestBlockReceivedAt),
    [setLatestBlockNumber]: (state, { payload: { latestBlockNumber, latestBlockReceivedAt } }) =>
      state.update('latestBlockNumber', () => latestBlockNumber).set('latestBlockReceivedAt', latestBlockReceivedAt),
