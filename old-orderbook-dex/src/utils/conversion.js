...
export function convertToTokenPrecisionInternal(precision, amountIn18Precision) {
  if (precision === APP_BASE_PRECISION) {
    ...
    const precisionDifference = APP_BASE_PRECISION - precision;
    return new BigNumber(amountIn18Precision).times(new BigNumber(10).pow(-precisionDifference)).toFixed(0);
  }
}
...
export function convertTo18PrecisionInternal(precision, amountInTokenPrecision) {
  if (precision === APP_BASE_PRECISION) {
    ...
    return new BigNumber(amountInTokenPrecision)
      .times(new BigNumber(10).pow(APP_BASE_PRECISION - precision))
      .toFixed(0);
  }
}

export function convertTo18Precision(amountInTokenPrecision, token) {
  return convertTo18PrecisionInternal(tokens.getTokenSpecs(getState())(token).get('precision'), amountInTokenPrecision);
}
