...
each([
  ...
  describe(`precision: ${precision}, token amount: ${tokenAmount}, 18 amount: ${p18Amount}`, () => {
    ...
    test(`cross check from 18 precision`, () => {
      expect(convertTo18PrecisionInternal(precision, convertToTokenPrecisionInternal(precision, p18Amount))).toEqual(
        p18Amount,
      );
    });

    test(`cross check from token precision`, () => {
      expect(convertToTokenPrecisionInternal(precision, convertTo18PrecisionInternal(precision, tokenAmount))).toEqual(
        tokenAmount,
      );
    });
  });
