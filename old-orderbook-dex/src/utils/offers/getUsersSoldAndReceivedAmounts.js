...
const getUsersSoldAndReceivedAmounts = (offerType, offerFormValues) => {
  if (
    !offerFormValues ||
    !offerFormValues.get('price') ||
    !offerFormValues.get('total') ||
    !offerFormValues.get('volume')
  ) {
  ...
  switch (offerType) {
    case TAKE_SELL_OFFER:
      return {
        amountReceived: formatAmount(volumeBN, false, null, 5),
        amountSold: formatAmount(totalBN, false, null, 5),
      };
    case TAKE_BUY_OFFER:
      return {
        amountReceived: formatAmount(totalBN, false, null, 5),
        amountSold: formatAmount(volumeBN, false, null, 5),
      };
